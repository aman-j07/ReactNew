...
const CurrencyConvertor = () => {
    const arrRates=Object.keys(rates);
  ...
  return (
    <div id="box">
        ...
        <div id="divCurrency"><div><label>From</label><select id="selFrom">{arrRates.map((item)=>{return <option>{item}</option>})}</select></div><i className="fa-solid fa-arrow-right-arrow-left"></i><div><label>To</label><select id="selTo">{arrRates.map((item)=>{return <option>{item}</option>})}</select></div></div>
